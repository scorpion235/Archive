...
char *postfix(char sim[MAX_SIZE] , int kol_sim, char tree[MAX_SIZE][MAX_SIZE])
{
  ...
  int i, j, k,   //счётчики
      ...
      kol_op,    //количество математических операций
  ...
  return (postfix(segm[0], kol_sim, tree)); //рекурсивный вызов функции "postfix()"
}
//------------------------------------------------------------------------------
//основная функция
int main()
{
  ...
  for (i=0;i<MAX_SIZE;i++)
    for (j=0;j<MAX_SIZE;j++)
      tree[i][j]='x';
  postfix(sim, kol_sim, tree); //преобразования префиксной записи числа в постфиксную
  getch();
  return 0;
}
